    ...
    try:
        ...
        while not all_workflow_runs_complete:
            ...
            for workflow_run in submission_state.workflow_runs:
                ...
                if workflow_run.status == SubmissionStateWorkflowRun.STATUS_FINISH_SUCCESS and workflow_run.validation_status == SubmissionStateWorkflowRun.VALIDATION_UNSTARTED:
        ...
        fail_n = 0
        for workflow_run in submission_state.workflow_runs:
            if workflow_run.status == SubmissionStateWorkflowRun.STATUS_FINISH_SUCCESS and workflow_run.validation_status == SubmissionStateWorkflowRun.VALIDATION_SUCCESS:
                # success condition
                pass
            elif workflow_run.status == SubmissionStateWorkflowRun.STATUS_FINISH_FAIL:
                fail_n += 1
                print(f"run with id '{workflow_run.wes_run_id}' failed execution with a state of {workflow_run.wes_state}")
            elif workflow_run.status == SubmissionStateWorkflowRun.STATUS_FINISH_SUCCESS and workflow_run.validation_status == SubmissionStateWorkflowRun.VALIDATION_FAIL:
                fail_n += 1
                print(f"run with id '{workflow_run.wes_run_id}' failed output validation. message: {workflow_run.validation_message}")
            else:
                fail_n += 1
                print(f"run with id '{workflow_run.wes_run_id}' failed with an unspecified error")

        if fail_n > 0:
            raise WdlTestCliExitException(f"{fail_n} workflow run(s) failed at the execution and/or validation stage", 1)

        # TODO add successful finish message
        print("Monitoring and validation complete. All workflow runs completed successfully and output the expected results")
